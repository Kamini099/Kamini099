# Hi there! 👋 I'm Kamini Kaushal Singh
<figure>
  <img align="left" alt="." src="https://github.com/Kamini099/Kamini099/blob/main/Little.png">
  <figcaption>
🔭 I’m currently working on an AI ML web project aimed at bringing justice through innovative technology solutions.

🌱 I’m currently learning about Blockchain and its applications in various industries.

💬 Ask me about my journey in web development, my experiences with different programming languages, or anything related to technology!

📫 How to reach me:
- Email: kam123inisingh@gmail.com
- LinkedIn: [Kamini Kaushal Singh](https://www.linkedin.com/in/kamini-kaushal-singh-646068232/)
- GitHub: [Kamini099](https://github.com/Kamini099)
  </figcaption>
</figure>

<table>
  <tr>
    <td><img src="https://github-readme-stats.vercel.app/api?username=Kamini099&theme=dracula&show_icons=true&hide_border=true&count_private=true" alt="Kamini099's GitHub Stats" /></td>
    <td><img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Kamini099&theme=dracula&show_icons=true&hide_border=true&layout=compact" alt="Kamini099's Top Languages" /></td>
  </tr>
</table>

😄 Pronouns: She/Her

⚡ Fun fact: I'm a member of the college Badminton team and enjoy exploring new dance forms in my free time!

## 🚀 Projects

- **SAMP (Student Academic & Mentorship Portal)**  
   - An official college project aimed at creating a portal for mentors and mentees to facilitate academic guidance.  
   - **Tools & Technologies:** HTML, CSS, React, Node.js, MongoDB  

- **Health Bridge**  
   - A comprehensive platform designed to support college students in raising funds for medical assistance.  
   - **Tools & Technologies:** HTML, CSS, Bootstrap, React, Agora API, Firebase  

- **Intern Tracker Chrome Extension**  
   - A dynamic Chrome extension designed to streamline internship applications.  
   - **Tools & Technologies:** JavaScript, HTML, CSS, Chrome APIs  

## 💻 Technical Skills

- **Languages:** C, C++, Python, HTML, CSS, JavaScript, SQL
- **Frameworks & Libraries:** React, Node.js, Express.js, Bootstrap
- **Databases:** MongoDB
- **Developer Tools:** Git, GitHub, Figma, Webflow, VSCode, Atom
- **APIs:** RESTful API
- **Soft Skills:** Communication, Project Management, Leadership

## 🎉 Achievements

- 560+ points on GeeksforGeeks
- 500+ questions solved on LeetCode
- Solved 300+ DSA questions
- Selected participant in the All India Girls Hackathon

## 🎓 Education

<table>
  <tr>
    <td><strong>B.Tech in Information Technology</strong><br>National Institute of Technology, Jalandhar<br>(2021-2025)<br>CGPA: 7.42</td>
    <td><strong>12th CBSE, Punjab</strong><br>Jawahar Navodaya Vidhyalaya, Pathankot<br>(2020-2021)<br>Percentage: 91%</td>
    <td><strong>10th CBSE, Punjab</strong><br>Jawahar Navodaya Vidhyalaya, Pathankot<br>(2018-2019)<br>Percentage: 91%</td>
  </tr>
</table>


## 🌐 Interests and Activities

- **Areas of Interest:** Web Development, Web Designing, MERN Stack
- **Hobbies:** Reading, Story Writing, Dancing

Feel free to connect with me! 😊
